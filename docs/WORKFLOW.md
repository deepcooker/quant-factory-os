...
- Runtime dispatcher note:
  - `tools/ops` is the stable CLI entrypoint (Python thin dispatcher).
  - Python-first commands currently include: `init/learn/ready/orient/choose/council/arbiter/slice`.
  - Non-migrated commands are temporarily compatibility-routed via `tools/ops_legacy.sh`.
...
- `S0 Environment`: `tools/ops init`
  ...
  - Implementation: Python-first (`tools/ops_init.py`), `tools/ops` keeps CLI wrapper only.
  ...
  - Modes:
    - default (`tools/ops init`): check + recommendation output (`INIT_STATUS` + `INIT_NEXT`)
...
- `S1.6 Learn Gate`: `tools/ops learn`
  ...
  - Implementation: Python-first (`tools/ops_learn.py`), `tools/ops` keeps CLI wrapper only.
  - Output visibility: prints `LEARN_STEP[<i>/<n>]` stage markers.
  - Stdout mirror is enabled by default: `tools/ops learn` writes `learn/{project_id}.stdout.log`.
    - one-shot disable: `QF_LEARN_LOG=0 tools/ops learn`
    - custom log path: `tools/ops learn LOG=<path>`
...
- `S2 Ready gate`: `tools/ops ready`
  - Input: restatement fields (goal/scope/acceptance/steps/stop)
  - Output: `reports/{RUN_ID}/ready.json`
  - Implementation: Python-first (`tools/ops_ready.py`), `tools/ops` keeps CLI wrapper only.
  ...
  - Learn dependency: `QF_READY_REQUIRE_LEARN=auto` (default) enforces learn gate by default.
    - auto recovery: `QF_READY_AUTO_LEARN=1` auto-runs `tools/ops learn` when learn evidence is missing/expired.
  ...
  - Exit resolution gate: if unresolved run context is detected, must choose:
    - `DECISION=resume-close` (run `tools/ops resume`)
  ...
  - Gate: `tools/ops do` must fail without valid `ready.json`.
- `S2.4 Plan protocol gate` (discussion-first, required for complex changes)
  - Interactive planning command: Codex `/plan` (not `tools/ops plan`).
...
- `S2.5 Direction gate`: `tools/ops orient` + `tools/ops choose`
...
- `S2.6 Council gate`: `tools/ops council`
...
- `S2.7 Arbiter gate`: `tools/ops arbiter`
...
- `S2.8 Slice gate`: `tools/ops slice`
...
- `S2.9 Discuss shortcut`: `tools/ops discuss`
...
- `tools/ops plan [N]` (legacy helper)
...
- `S3 Execute`: `tools/ops do queue-next`
  ...
  - Auto checkpoint: runs `tools/ops review RUN_ID=<picked-run> AUTO_FIX=1 NON_BLOCKING=1` to emit drift report early.
- `S2.5~S3 Orchestrator (optional)`: `tools/ops execute`
  ...
  - Contract confirm gate for execution:
    ...
    - quick confirm command: `tools/ops execute RUN_ID=<run-id> PROJECT_ID=<project-id> CONFIRM_CONTRACT=1 TARGET=do`.
    - automation mode: `QF_EXECUTE_AUTO_CONFIRM_CONTRACT=1 tools/ops execute`.
  - Auto mode: `QF_EXECUTE_AUTO_CHOOSE=1 tools/ops execute` uses orient recommended option and continues through `council->arbiter->slice->do`.
- `S3.5 Review`: `tools/ops review`
...
- For anti-loss fallback, store concise session checkpoints in
  ...
  - `tools/ops snapshot RUN_ID=<run-id> NOTE="decision/next-step summary"`
- `/compact` policy:
...
- `tools/ops do` / `tools/ops resume` 自动记录执行轨迹到
  `reports/{RUN_ID}/execution.jsonl`（默认脱敏，可审计）。
- `tools/ops resume` 在同步回 `main` 前若检测到脏工作区，会自动 stash
  `ops-resume-cleanup-run-{RUN_ID}-wip-*`，避免因自身日志写入导致 checkout 自阻塞。
- `tools/ops ready` / `tools/ops orient` / `tools/ops choose` /
  `tools/ops council` / `tools/ops arbiter` / `tools/ops slice` 默认写入
...
- 断线恢复建议先生成接班摘要：
  - `tools/ops handoff RUN_ID=<run-id>` -> `reports/{RUN_ID}/handoff.md`
...
- `tools/ops init` completed successfully.
- `tools/ops learn` produced valid `learn/{project_id}.json` (model sync mandatory; `RUN_ID` not required).
- `tools/ops ready` produced `reports/{RUN_ID}/ready.json`.
- `tools/ops orient` produced `chatlogs/discussion/{RUN_ID}/orient.json`.
- `tools/ops choose` produced `reports/{RUN_ID}/orient_choice.json`.
- `tools/ops council` produced `chatlogs/discussion/{RUN_ID}/council.json`.
- `tools/ops arbiter` produced `reports/{RUN_ID}/execution_contract.json`.
- `tools/ops slice` produced `reports/{RUN_ID}/slice_state.json`.
- `tools/ops do queue-next` requires all gates above and then picks via `tools/task.sh --next`.
...
- Optional shortcut: `tools/ops execute` can run the same chain with explicit/auto option strategy.
...
- Preferred entrypoint: `tools/ops` (`init/learn/ready/orient/choose/council/arbiter/slice/execute/do/review/resume`).
- Compatibility wrappers: `tools/enter.sh` and `tools/onboard.sh` forward to `tools/ops`.
- 1) 运行 `tools/ops init`（环境体检；不自动改工作区，不自动 handoff）。
- 1.1) 只看状态可用：`tools/ops init -status`（抑制 resume 提示文案）。
- 1.2) 强制 main 约束可用：`tools/ops init -main`（不满足即失败）。
- 1.0) 若希望在终端消费结构化日志，可设置 `QF_EVENT_STREAM=1`（stdout 会追加 JSONL 事件）。
- 2) 若 `INIT_STATUS=needs_resume`/`blocked`，先执行 `tools/ops resume RUN_ID=<run-id>` 处理收尾问题。
- 2.1) `handoff` 改为显式动作：`tools/ops handoff RUN_ID=<run-id>`（按需调用）。
- 3) 按顺序阅读并复述：`AGENTS.md` -> `docs/PROJECT_GUIDE.md` -> `docs/WORKFLOW.md` -> `docs/ENTITIES.md` -> `TASKS/STATE.md` -> `TASKS/QUEUE.md`。
- 3.1) 运行 `tools/ops learn` 固化“上岗学习”证据（项目+宪法+工作流+技能+session），默认打印分步日志。
...
- 4) 运行 `tools/ops ready` 完成复述上岗门禁（默认绑定 `CURRENT_RUN_ID`，默认可自动填充；默认缺失 learn 时可自动补跑）。
- 4.0) 若 `ready` 提示 unresolved run context，先二选一：
  - `tools/ops resume RUN_ID=<run-id>`（收尾）
  - `tools/ops ready RUN_ID=<run-id> DECISION=abandon-new`（明确抛弃旧上下文后继续）
- 4.1) 运行 `tools/ops orient` 生成方向候选与优先级（L1 方向层）。
- 4.2) 运行 `tools/ops choose OPTION=<id>` 确认方向后再进入执行层（L2）。
- 4.2.1) 运行 `tools/ops council` 生成产品/架构/研发/测试独立评审结果（讨论态）。
- 4.2.2) 运行 `tools/ops arbiter` 收敛为统一执行契约（执行态）。
- 4.2.3) 运行 `tools/ops slice` 把执行契约拆成最小 queue tasks（幂等入队）。
- 4.2.4) 低摩擦讨论收敛可用：`tools/ops discuss`（默认停在 prepare，不直接 do）。
- 4.3) 在关键决策点执行 `tools/ops snapshot NOTE="..."`，把“本轮结论/下一步”写入仓库证据，避免会话丢失。
- 5) 运行 `tools/ops do queue-next` 领取下一枪（内部强制 ready + choose + council + arbiter + slice 前置；并自动产出一次 non-blocking drift review）。
- 5.0) 低摩擦可选：`tools/ops execute`
  - 默认在缺少方向确认时停在 choose（保留人工确认）
  - 自动推进模式：`QF_EXECUTE_AUTO_CHOOSE=1 tools/ops execute`
- 5.1) 需求执行完成后，显式运行 `tools/ops review RUN_ID=<run-id> STRICT=1 AUTO_FIX=1`，清空 blocker 后再 ship。
...
- Ship failure recovery: `tools/ship.sh` writes `reports/{RUN_ID}/ship_state.json`
  at key steps. On push/PR/merge/sync failure, run `tools/ops resume RUN_ID=...`.
- `tools/ops resume` 会先检查是否已存在同分支的已合并 PR；若已合并则跳过重复 `pr create/merge`，直接执行本地 `main` 同步收尾。
