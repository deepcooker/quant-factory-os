...
# gitclient 中文：等待 PR 真正进入 MERGED 状态，只有合并完成后才允许同步本地 main。
def wait_for_pr_merged(pr_url: str, repo_path: Path) -> dict[str, Any]:
    deadline = time.time() + PR_MERGE_WAIT_TIMEOUT_SEC
    last_state = ""
    last_merge_state = ""
    while time.time() < deadline:
        view_proc = run_cmd(["gh", "pr", "view", pr_url, "--json", "state,mergedAt,mergeStateStatus,autoMergeRequest"], repo_path)
        if view_proc.returncode != 0:
            return command_err(ERR_GH_COMMAND_FAILED, "查询PR状态失败", view_proc)
        try:
            payload = json.loads(view_proc.stdout or "{}")
        except json.JSONDecodeError:
            return err(ERR_GH_COMMAND_FAILED, "PR状态返回不是合法JSON", {"stdout": view_proc.stdout.strip()})
        state = str(payload.get("state", "")).strip()
        merged_at = str(payload.get("mergedAt", "")).strip()
        merge_state = str(payload.get("mergeStateStatus", "")).strip()
            ...
            return ok(
                {
                    ...
                    "auto_merge_enabled": bool(auto_merge_request),
                }
            )
    ...
    return err(
        ...
        "等待PR合并超时",
        {
            "pr_url": pr_url,
            "state": last_state,
            "merge_state_status": last_merge_state,
        },
    )


# gitclient 中文：发起 PR 合并，优先直接合并；若受分支策略限制，则自动切换为 auto merge 并等待真合并。
def merge_pr_and_wait(pr_url: str, repo_path: Path) -> dict[str, Any]:
    ...
    stderr_text = (direct_merge_proc.stderr or "").strip()
    if "add the `--auto` flag" not in stderr_text:
        return command_err(ERR_GH_COMMAND_FAILED, "合并PR失败", direct_merge_proc)

    auto_merge_proc = run_cmd(["gh", "pr", "merge", pr_url, "--auto", "--squash", "--delete-branch"], repo_path)
    if auto_merge_proc.returncode != 0:
        return command_err(ERR_GH_COMMAND_FAILED, "设置PR自动合并失败", auto_merge_proc)
...
def run_commit_and_merge(message: str, base_branch: str = "main") -> dict[str, Any]:
    ...
    merge_result = merge_pr_and_wait(pr_url, repo_path)
    if merge_result["err_code"] != 0:
        return merge_result
    ...
    return ok(
        {
            ...
            "pr_url": pr_url,
            "merge_mode": merge_result["data"].get("merge_mode", ""),
            "merged_at": merge_result["data"].get("merged_at", ""),
        }
    )
...
def run_rollback_commit(commit_sha: str, base_branch: str = "main") -> dict[str, Any]:
    ...
    if merge_result["err_code"] != 0:
        return err(
            merge_result["err_code"],
            "合并回滚PR失败",
            merge_result.get("data", {}),
        )
