...
#### 标准答案
最近 session 的内容应该从仓库证据里查，不应该靠聊天记忆猜。最重要的来源是 `tools/project_config.json -> runtime_state`、当前 `RUN_ID` 下的 `reports/<RUN_ID>/summary.md` 和 `decision.md`，必要时再看 `conversation.md`。`TASKS/STATE.md` 只是 runtime_state 的镜像输出。根据当前证据，最近 session 的主线是：继续压缩流程噪音，把 `PROJECT_GUIDE` 做成真正的高质量提问课程，把模型通过口述与锚点拉回项目主线，并把长期形态收敛成“普通窗口执行 `tools`，Codex 通过 app-server 提供运行时智能”。
#### 必查文件
- tools/project_config.json
- reports/<RUN_ID>/summary.md
- reports/<RUN_ID>/decision.md
