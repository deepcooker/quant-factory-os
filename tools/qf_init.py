...
def parse_args(argv: list[str]) -> str:
    ...
    for arg in argv:
        if arg == "-status":
            ...
            eprint("Usage: tools/qf init [-status|-main]")
            raise SystemExit(2)
    return init_mode
...
def main(argv: list[str]) -> int:
    ...
    next_cmd = "tools/qf learn"
    if init_status != "ok":
        next_cmd = f"tools/qf resume RUN_ID={current_run_id}" if current_run_id else "tools/qf resume"
    ...
    if init_mode == "check":
        print("== 建议流程 ==")
        print("1) tools/qf learn")
        print("2) tools/qf ready")
        print("3) tools/qf discuss TARGET=prepare")
        print("4) tools/qf do queue-next")
    else:
        print("== 状态查询完成 ==")
    return 0
