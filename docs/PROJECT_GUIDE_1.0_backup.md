...
#### 标准答案
要看当前有没有未完成任务，首先看 `tools/project_config.json -> runtime_state` 当前指针，再看 `TASKS/QUEUE.md` 队列，再看当前 `RUN_ID` 下的 `summary.md` 和 `decision.md`。`TASKS/STATE.md` 现在只是 runtime_state 的镜像输出。当前项目最近讨论的重点，不是新业务功能，而是自动化流程本身，尤其是 `learn` 是否真的完成同频、`PROJECT_GUIDE` 如何成为学习课程、以及流程能否更简单、更自动化。
#### 必查文件
- tools/project_config.json
- TASKS/QUEUE.md
- reports/<RUN_ID>/summary.md
...
#### 标准答案
最近 session 的内容应该从仓库证据里查，不应该靠聊天记忆猜。最重要的来源是 `tools/project_config.json -> runtime_state`、当前 `RUN_ID` 下的 `reports/<RUN_ID>/summary.md` 和 `decision.md`，必要时再看 `conversation.md`。`TASKS/STATE.md` 只是 runtime_state 的镜像输出。根据当前证据，最近 session 的主线是：继续压缩流程噪音，把 `learn` 做成真正的项目同频核心，并用 `PROJECT_GUIDE` 把模型拉回主线，而不是让它在单个技术细节里打转。
#### 必查文件
- tools/project_config.json
- reports/<RUN_ID>/summary.md
- reports/<RUN_ID>/decision.md
...
#### 标准答案
`project` 是最高层项目维度，当前基座默认是 `quant-factory-os`；`run` 是一次执行或讨论周期对应的证据命名空间，主要落在 `reports/<RUN_ID>/`；`task` 是当前要做的具体合同，落在 `TASKS/TASK-*.md`；`PR` 是交付与审查单元。除此之外，还有 `queue` 作为待办入口，`gate` 作为门禁，`evidence` 作为仓库内记忆。生命周期一般是：`QUEUE -> TASK -> RUN evidence -> verify -> ship/PR -> state update`。
#### 必查文件
- docs/ENTITIES.md
- docs/WORKFLOW.md
- AGENTS.md
