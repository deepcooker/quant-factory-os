...
def main(argv: list[str]) -> int:
    ...
    if not run_id:
        ...
        eprint("Usage: tools/qf ready [RUN_ID=<run-id>] [DECISION=resume-close|abandon-new]")
        return 2
    ...
    if require_learn == "1":
        ...
        if not learn_report_file and auto_learn == "1":
            print("LEARN_AUTO_RUN: tools/qf learn")
            cp = run_cmd(["bash", "tools/qf", "learn", f"PROJECT_ID={project_id}"])
            if cp.stdout:
        ...
        if not learn_report_file:
            ...
            eprint(f"Run: tools/qf learn PROJECT_ID={project_id}")
            eprint(f"Then retry: tools/qf ready RUN_ID={run_id}")
            return 1
    ...
    if require_sync == "1":
        if not sync_report_file and auto_sync == "1":
            print(f"SYNC_AUTO_RUN: tools/qf sync RUN_ID={run_id}")
            cp = run_cmd(["bash", "tools/qf", "sync", f"RUN_ID={run_id}"])
            if cp.stdout:
                sys.stdout.write(cp.stdout)
            if cp.stderr:
                sys.stderr.write(cp.stderr)
        ...
        if not sync_report_file:
            ...
            eprint(f"Run: tools/qf sync RUN_ID={run_id}")
            eprint(f"Then retry: tools/qf ready RUN_ID={run_id}")
            return 1
    ...
    if resolution_required == 1:
        if not continue_decision:
            ...
            print(f"READY_NEXT_1: tools/qf resume RUN_ID={run_id}")
            print(f"READY_NEXT_2: tools/qf ready RUN_ID={run_id} DECISION=abandon-new")
            return 1
        if continue_decision == "resume-close":
            print("READY_DECISION: resume-close")
            print(f"READY_NEXT_COMMAND: tools/qf resume RUN_ID={run_id}")
            return 1
