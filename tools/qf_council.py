...
def main(argv: list[str]) -> int:
    ...
    if not choice_file.is_file():
        ...
        print(f"Run: tools/qf orient RUN_ID={run_id}", file=sys.stderr)
        print(f"Then: tools/qf choose RUN_ID={run_id} OPTION=<id>", file=sys.stderr)
        print("Then retry: tools/qf do queue-next", file=sys.stderr)
        return 1
    if not contract_json.is_file():
        print(f"ERROR: missing direction contract: {contract_json}", file=sys.stderr)
        print(f"Run: tools/qf choose RUN_ID={run_id} OPTION=<id>", file=sys.stderr)
        return 1
    ...
    if disagreements:
        ...
        lines.append("- none")
    lines.extend(["", "## Next Command", f"- `tools/qf arbiter RUN_ID={run_id}`", ""])
    ...
    append_execution_event(
        ...
        "council_generated",
        "ok",
        f"tools/qf council RUN_ID={run_id}",
