...
usage() {
  cat <<'EOF'
Usage:
  tools/ops init [-status|-main]
  tools/ops onboard RUN_ID
  tools/ops sync [RUN_ID=<run-id>]
  tools/ops learn [PROJECT_ID=<project-id>] [TTL_DAYS=<n>] [PLAN_TRANSPORT=auto|slash] [MODEL=<slug>] [-log] [LOG=<path>]
  tools/ops ready [RUN_ID=<run-id>] [DECISION=resume-close|abandon-new]
  tools/ops orient [RUN_ID=<run-id>]
  tools/ops choose [RUN_ID=<run-id>] OPTION=<id>
  tools/ops council [RUN_ID=<run-id>]
  tools/ops arbiter [RUN_ID=<run-id>]
  tools/ops slice [RUN_ID=<run-id>]
  tools/ops discuss [RUN_ID=<run-id>] [PROJECT_ID=<project-id>] [OPTION=<id>] [AUTO_CHOOSE=0|1] [TARGET=prepare|do] [CONFIRM_CONTRACT=0|1] [AUTO_CONFIRM_CONTRACT=0|1]
  tools/ops execute [RUN_ID=<run-id>] [PROJECT_ID=<project-id>] [OPTION=<id>] [AUTO_CHOOSE=0|1] [TARGET=prepare|do] [CONFIRM_CONTRACT=0|1] [AUTO_CONFIRM_CONTRACT=0|1]
  tools/ops review [RUN_ID=<run-id>] [AUTO_FIX=0|1] [STRICT=0|1]
  tools/ops snapshot [RUN_ID=<run-id>] NOTE=<text>
  tools/ops handoff [RUN_ID=<run-id>]
  tools/ops exam [RUN_ID=<run-id>] [ANSWER_FILE=<path>] [RUBRIC_FILE=<path>] [OUTPUT_FILE=<path>]
  tools/ops exam-auto [RUN_ID=<run-id>] [ANSWER_FILE=<path>] [RUBRIC_FILE=<path>] [OUTPUT_FILE=<path>] [AUTO_FILL=0|1]
  tools/ops plan [N]
  tools/ops do queue-next
  tools/ops resume [RUN_ID=<run-id>]
  tools/ops stash-clean [preview|apply] [KEEP=<n>]
...
cmd_onboard() {
  ...
  if [[ -z "$run_id" ]]; then
    echo "ERROR: RUN_ID is required. Usage: tools/ops onboard <RUN_ID>" >&2
    exit 2
  fi
...
require_council_gate() {
  ...
  if [[ -z "$council_file" ]]; then
    ...
    echo "Run: tools/ops council RUN_ID=${run_id}" >&2
    echo "Then: tools/ops arbiter RUN_ID=${run_id}" >&2
    exit 1
  fi
...
require_direction_gate() {
  ...
  if [[ ! -f "$choice_file" ]]; then
    ...
    echo "Run: tools/ops orient RUN_ID=${run_id}" >&2
    echo "Then: tools/ops choose RUN_ID=${run_id} OPTION=<id>" >&2
    echo "Then retry: tools/ops do queue-next" >&2
    exit 1
  fi
...
require_execution_contract_gate() {
  ...
  if [[ ! -f "$file" ]]; then
    ...
    echo "Run: tools/ops arbiter RUN_ID=${run_id}" >&2
    echo "Then: tools/ops slice RUN_ID=${run_id}" >&2
    echo "Then retry: tools/ops do queue-next" >&2
    exit 1
  fi
...
require_slice_gate() {
  ...
  if [[ ! -f "$file" ]]; then
    ...
    echo "Run: tools/ops slice RUN_ID=${run_id}" >&2
    echo "Then retry: tools/ops do queue-next" >&2
    exit 1
  fi
...
if orient_path.is_file():
    try:
        ...
        recommended_option = ""
if not learn_exists:
    next_command = "tools/ops learn"
elif not ready_exists:
    next_command = "tools/ops ready"
elif not choose_exists:
    if orient_path.is_file() and recommended_option:
        next_command = f"tools/ops choose RUN_ID={run_id} OPTION={recommended_option}"
    elif orient_path.is_file():
        next_command = f"tools/ops choose RUN_ID={run_id} OPTION=<id>"
    else:
        next_command = f"tools/ops orient RUN_ID={run_id}"
elif not council_exists:
    next_command = f"tools/ops council RUN_ID={run_id}"
    ...
    next_command = "tools/ops do queue-next"
...
obj = {
    ...
    "next_command_low_friction": (
        f"tools/ops execute RUN_ID={run_id}"
        if ready_exists
        else ("tools/ops ready" if learn_exists else "tools/ops learn")
    ),
}
...
PY
  ...
  append_execution_event "$run_id" "sync" "sync_generated" "ok" "tools/ops sync RUN_ID=${run_id}" "sync_file=${sync_file};sync_md=${sync_md}" ""
...
cmd_learn() {
  ...
  if command -v python3 >/dev/null 2>&1; then
    ...
    echo "ERROR: python is required for tools/ops learn." >&2
    exit 1
  fi
...
lines.append("- Do not call tools/ops init/learn/ready inside this model-sync pass.")
...
PY
  ...
  echo "LEARN_NEXT_COMMAND: tools/ops ready"
}
...
cmd_ready() {
  ...
  if [[ -z "$run_id" ]]; then
    ...
    echo "Usage: tools/ops ready [RUN_ID=<run-id>] [DECISION=resume-close|abandon-new]" >&2
    exit 2
  fi
  ...
  if [[ "$require_learn" == "1" ]]; then
    ...
    if [[ -z "$learn_report_file" && "$auto_learn" == "1" ]]; then
      echo "LEARN_AUTO_RUN: tools/ops learn"
      cmd_learn "PROJECT_ID=${project_id}"
      learn_report_file="$(resolve_learn_file_for_project "$project_id" || true)"
    fi
    if [[ -z "$learn_report_file" ]]; then
      echo "ERROR: learn gate not satisfied." >&2
      echo "Run: tools/ops learn PROJECT_ID=${project_id}" >&2
      echo "Then retry: tools/ops ready RUN_ID=${run_id}" >&2
      exit 1
    fi
  fi
  ...
  if [[ "$resolution_required" -eq 1 ]]; then
    if [[ -z "$continue_decision" ]]; then
      ...
      echo "READY_NEXT_1: tools/ops resume RUN_ID=${run_id}"
      echo "READY_NEXT_2: tools/ops ready RUN_ID=${run_id} DECISION=abandon-new"
      exit 1
    fi
    case "$continue_decision" in
      resume-close)
        echo "READY_DECISION: resume-close"
        echo "READY_NEXT_COMMAND: tools/ops resume RUN_ID=${run_id}"
        exit 1
        ;;
      abandon-new)
        ;;
...
PY
  ...
  append_execution_event "$run_id" "ready" "ready_passed" "ok" "tools/ops ready" "ready_file=${ready_file};learn_report=${learn_report_file};sync_report=${sync_report_file};task_file=${task_file};brief=${ready_brief_md};orient=${orient_file}" ""
...
cmd_orient() {
  ...
  append_execution_event "$run_id" "orient" "orient_generated" "ok" "tools/ops orient RUN_ID=${run_id}" "orient_file=${orient_file};orient_md=${orient_md}" ""
...
contract = {
    ...
    "next_command": "tools/ops council RUN_ID={}".format(obj.get("run_id") or ""),
}
out = {
    "project_id": project_id,
    "run_id": obj.get("run_id"),
...
lines.append("")
lines.append("## Next Command")
lines.append(f"- `tools/ops council RUN_ID={obj.get('run_id')}`")
lines.append("")
...
PY
  ...
  append_execution_event "$run_id" "orient" "orient_chosen" "ok" "tools/ops choose RUN_ID=${run_id} OPTION=${option}" "choice_file=${choice_file};contract=${contract_json}" ""
...
cmd_council() {
  ...
  if [[ ! -f "$contract_json" ]]; then
    ...
    echo "Run: tools/ops choose RUN_ID=${run_id} OPTION=<id>" >&2
    exit 1
  fi
...
lines.append("")
lines.append("## Next Command")
lines.append(f"- `tools/ops arbiter RUN_ID={run_id}`")
lines.append("")
Path(out_md).write_text("\n".join(lines), encoding="utf-8")
...
PY
  ...
  append_execution_event "$run_id" "council" "council_generated" "ok" "tools/ops council RUN_ID=${run_id}" "choice_file=${choice_file};council=${council_json}" ""
...
lines.append("")
lines.append("## Next Command")
lines.append(f"- `tools/ops slice RUN_ID={run_id}`")
lines.append("")
Path(out_md).write_text("\n".join(lines), encoding="utf-8")
...
PY
  ...
  append_execution_event "$run_id" "arbiter" "arbiter_generated" "ok" "tools/ops arbiter RUN_ID=${run_id}" "council_file=${council_file};execution_contract=${out_json}" ""
...
print("SLICE_NEXT_COMMAND: tools/ops do queue-next")
...
PY
  ...
  append_execution_event "$run_id" "slice" "slice_generated" "ok" "tools/ops slice RUN_ID=${run_id}" "execution_contract=${execution_contract};slice_state=${out_json}" ""
...
cmd_execute() {
  ...
  append_execution_event "$run_id" "execute" "execute_start" "ok" "tools/ops execute RUN_ID=${run_id}" "project_id=${project_id};option=${option};auto_choose=${auto_choose};confirm_contract=${confirm_contract};auto_confirm_contract=${auto_confirm_contract};target=${target}" ""
  ...
  if [[ -z "$orient_file" || ! -f "$orient_file" ]]; then
    append_execution_event "$run_id" "execute" "execute_orient_missing" "fail" "tools/ops execute RUN_ID=${run_id}" "" "orient file missing after orient step"
    ...
    if [[ -z "$option" ]]; then
      if [[ "$auto_choose" == "1" && -n "$recommended" ]]; then
        ...
        if [[ -n "$recommended" ]]; then
          ...
          echo "EXECUTE_NEXT_COMMAND: tools/ops choose RUN_ID=${run_id} OPTION=${recommended}"
        else
          echo "EXECUTE_NEXT_COMMAND: tools/ops choose RUN_ID=${run_id} OPTION=<id>"
        fi
        append_execution_event "$run_id" "execute" "execute_wait_option" "fail" "tools/ops execute RUN_ID=${run_id}" "recommended=${recommended}" "missing option confirmation"
        exit 1
      fi
    fi
...
PY
  ...
  if [[ "$target" == "do" && ! -f "$contract_confirm_file" ]]; then
    ...
    echo "EXECUTE_NEXT_COMMAND: tools/ops execute RUN_ID=${run_id} PROJECT_ID=${project_id} CONFIRM_CONTRACT=1 TARGET=do"
    append_execution_event "$run_id" "execute" "execute_wait_contract_confirm" "fail" "tools/ops execute RUN_ID=${run_id}" "project_id=${project_id};contract_confirm_file=${contract_confirm_file}" "missing execution contract confirmation"
    exit 1
  fi

  if [[ "$target" == "prepare" ]]; then
    append_execution_event "$run_id" "execute" "execute_prepared" "ok" "tools/ops execute RUN_ID=${run_id} TARGET=prepare" "project_id=${project_id};contract_confirm_file=${contract_confirm_file}" ""
    echo "EXECUTE_STATUS: prepared"
    echo "EXECUTE_NEXT_COMMAND: tools/ops do queue-next"
    return 0
  fi

  echo "EXECUTE_STEP: do"
  RUN_ID="$run_id" cmd_do "queue-next"
  append_execution_event "$run_id" "execute" "execute_done" "ok" "tools/ops execute RUN_ID=${run_id}" "project_id=${project_id};contract_confirm_file=${contract_confirm_file}" ""
}
...
next_cmd = f"tools/ops do queue-next" if status == "pass" else f"tools/ops snapshot RUN_ID={run_id} NOTE=\"review blockers\""
...
PY
  ...
  if [[ "$rc" -ne 0 ]]; then
    append_execution_event "$run_id" "review" "review_failed" "fail" "tools/ops review RUN_ID=${run_id} AUTO_FIX=${auto_fix} STRICT=${strict}" "task_file=${task_file}" "$output"
    append_conversation_checkpoint "$run_id" "review" "review failed; see drift_review.md"
    return "$rc"
  fi

  append_execution_event "$run_id" "review" "review_passed" "ok" "tools/ops review RUN_ID=${run_id} AUTO_FIX=${auto_fix} STRICT=${strict}" "task_file=${task_file}" ""
...
if not ready:
    next_cmd = "tools/ops ready"
elif ship and ship.get("last_error"):
    next_cmd = "tools/ops resume"
elif not os.path.exists(choice_file):
    if os.path.exists(orient_file):
        next_cmd = f"tools/ops choose RUN_ID={run_id} OPTION=<id>"
    else:
        next_cmd = f"tools/ops orient RUN_ID={run_id}"
elif not os.path.exists(council_file):
    next_cmd = f"tools/ops council RUN_ID={run_id}"
elif not os.path.exists(execution_contract_file):
    next_cmd = f"tools/ops arbiter RUN_ID={run_id}"
elif not os.path.exists(slice_state_file):
    next_cmd = f"tools/ops slice RUN_ID={run_id}"
else:
    next_cmd = "tools/ops do queue-next"
...
if any((x.get("status") == "fail") for x in exec_tail):
    thinking_lines.append("- 最近出现失败事件，建议先 `tools/ops resume` 处理恢复，再继续新任务。")
elif missing:
    thinking_lines.append("- 接班信息有缺口；后续在关键决策点执行 `tools/ops snapshot NOTE=\"...\"` 提升连续性。")
else:
...
PY
  ...
  append_execution_event "$run_id" "handoff" "handoff_generated" "ok" "tools/ops handoff RUN_ID=${run_id}" "handoff_file=${out_file}" ""
  current_status="$(state_field_value "CURRENT_STATUS")"
  if [[ -z "$current_status" ]]; then
    current_status="active"
  fi
...
cmd_init() {
  ...
  local next_cmd="tools/ops learn"
  ...
  if [[ "$init_status" != "ok" ]]; then
    if [[ -n "$current_run_id" ]]; then
      next_cmd="tools/ops resume RUN_ID=${current_run_id}"
    else
      next_cmd="tools/ops resume"
    fi
  fi
  ...
  if [[ "$init_mode" == "check" ]]; then
    echo "== 建议流程 =="
    echo "1) tools/ops learn"
    echo "2) tools/ops ready"
    echo "3) tools/ops discuss TARGET=prepare"
    echo "4) tools/ops do queue-next"
  else
    echo "== 状态查询完成 =="
  fi
}
...
cmd_do() {
  ...
  if is_dirty; then
    ...
    append_execution_event "$run_id" "do" "do_sync_skipped_dirty" "ok" "tools/ops do queue-next" "reason=dirty_worktree" ""
  else
    sync_main "$run_id"
  fi
  append_execution_event "$run_id" "do" "do_start" "ok" "tools/ops do queue-next" "ready_file=${ready_file};choice_file=${choice_file};council_file=${council_file};execution_contract=${execution_contract};slice_state=${slice_state}" ""
  ...
  if [[ "$rc" -ne 0 ]]; then
    ...
    if [[ "$output" == *"QUEUE 中没有未完成项"* ]]; then
      echo "下一步建议：tools/ops orient RUN_ID=${run_id}" >&2
    fi
    print_resume_cmd "$run_id"
    exit "$rc"
  fi
  ...
  if [[ "${QF_DO_AUTO_REVIEW:-1}" == "1" ]]; then
    echo "AUTO_REVIEW: tools/ops review RUN_ID=${review_run_id} AUTO_FIX=${QF_DO_REVIEW_AUTO_FIX:-1}"
    set +e
    ...
    if [[ "$review_rc" -ne 0 ]]; then
      append_execution_event "$review_run_id" "do" "do_auto_review_failed" "fail" "tools/ops review RUN_ID=${review_run_id}" "" "$review_output"
      echo "WARN: auto review failed (non-blocking)."
    else
      append_execution_event "$review_run_id" "do" "do_auto_review_done" "ok" "tools/ops review RUN_ID=${review_run_id}" "" ""
    fi
  fi
}
...
cmd_stash_clean() {
  ...
  if [[ "$mode" != "apply" ]]; then
    echo "next: tools/ops stash-clean apply KEEP=${keep_latest}"
    return 0
  fi
  ...
  command_line="tools/ops stash-clean apply KEEP=${keep_latest}"
...
cmd_exam_auto_fill_answer() {
  ...
  if command -v python3 >/dev/null 2>&1; then
    ...
    echo "ERROR: python is required for tools/ops exam-auto." >&2
    exit 1
  fi
...
- 证据文件路径：`AGENTS.md`、`tools/ops`、`reports/{run_id}/conversation.md`
...
- 操作指南路径：`docs/WORKFLOW.md`
- 证据文件路径：`tools/ops`、`chatlogs/discussion/{run_id}/`
...
- 证据文件路径：`tools/ops`、`chatlogs/discussion/{run_id}/council.json`
...
- 讨论流程与入口：`tools/ops discuss` 或分步命令
- 证据文件路径：`docs/WORKFLOW.md`、`tools/ops`
...
- 证据文件路径：`tools/ops`、`docs/WORKFLOW.md`、`AGENTS.md`
...
cmd_exam() {
  ...
  if command -v python3 >/dev/null 2>&1; then
    ...
    echo "ERROR: python is required for tools/ops exam." >&2
    exit 1
  fi

  if [[ ! -f "$answer_file" ]]; then
    append_execution_event "$run_id" "exam" "exam_missing_answer" "fail" "tools/ops exam RUN_ID=${run_id}" "answer_file=${answer_file}" "missing answer file"
  ...
  append_execution_event "$run_id" "exam" "exam_graded" "$status" "tools/ops exam RUN_ID=${run_id}" "answer=${answer_file};rubric=${rubric_file};output=${output_file}" "$output"
  if [[ "$rc" -ne 0 ]]; then
    exit "$rc"
  fi
}
...
PY
    then
      ...
      append_execution_event "$run_id" "exam-auto" "exam_auto_answer_migrated" "ok" "tools/ops exam-auto RUN_ID=${run_id}" "answer_file=${answer_file}" ""
      echo "EXAM_ANSWER_AUTOFILLED: $answer_file"
    fi
  fi
  if [[ ! -f "$answer_file" ]]; then
    command_line="tools/ops exam-auto RUN_ID=${run_id}"
    if [[ "$auto_fill" == "0" ]]; then
      ...
      echo "请先按模板补全答案，再运行：tools/ops exam-auto RUN_ID=${run_id}"
      return 3
    fi
...
cmd_resume() {
  ...
  if [[ ! -f "$state_file" ]]; then
    append_execution_event "$run_id" "resume" "resume_missing_state" "fail" "tools/ops resume RUN_ID=${run_id}" "state_file=${state_file}" "missing state file"
    echo "ERROR: missing state file: $state_file"
    print_resume_cmd "$run_id"
    exit 1
  fi

  append_execution_event "$run_id" "resume" "resume_start" "ok" "tools/ops resume RUN_ID=${run_id}" "state_file=${state_file}" ""
  ...
  append_execution_event "$run_id" "resume" "resume_done" "ok" "tools/ops resume RUN_ID=${run_id}" "pr_url=${pr_url}" ""
