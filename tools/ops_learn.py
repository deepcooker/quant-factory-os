...
def parse_cli(argv: list[str]) -> dict[str, Any]:
    ...
    if plan_transport not in {"auto", "slash"}:
        eprint(f"ERROR: invalid QF_LEARN_PLAN_TRANSPORT={plan_transport_raw} (expected auto|slash).")
        raise SystemExit(2)
...
def run_slash_transport(prompt_file: Path, raw_file: Path, events_file: Path, stderr_file: Path, model_name: str) -> int:
    try:
...
def main(argv: list[str]) -> int:
    ...
    if cfg["plan_transport"] == "auto":
        ...
        if pty_capable:
            ...
            print("LEARN_MODEL_PLAN_TRANSPORT_AUTO_REASON: no_pty_devices_forced_slash")
