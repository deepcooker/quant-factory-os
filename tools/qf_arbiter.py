...
def main(argv: list[str]) -> int:
    ...
    if not council_file.is_file():
        ...
        print(f"Run: tools/qf council RUN_ID={run_id}", file=sys.stderr)
        print(f"Then: tools/qf arbiter RUN_ID={run_id}", file=sys.stderr)
        return 1
    ...
    for t in tasks:
        ...
        for a in t["acceptance"]:
            lines.append(f"    - {a}")
    lines.extend(["", "## Next Command", f"- `tools/qf slice RUN_ID={run_id}`", ""])
    ...
    append_execution_event(
        ...
        "arbiter_generated",
        "ok",
        f"tools/qf arbiter RUN_ID={run_id}",
