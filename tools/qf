...
usage() {
  cat <<'EOF'
Usage:
  tools/qf init [-status|-main]
  tools/qf onboard RUN_ID
  tools/qf sync [RUN_ID=<run-id>]
  tools/qf learn [PROJECT_ID=<project-id>] [TTL_DAYS=<n>] [PLAN_TRANSPORT=auto|slash|exec] [MODEL_TIMEOUT_SEC=<sec>] [MODEL=<slug>] [-log] [LOG=<path>]
  tools/qf ready [RUN_ID=<run-id>] [DECISION=resume-close|abandon-new]
  tools/qf orient [RUN_ID=<run-id>]
  tools/qf choose [RUN_ID=<run-id>] OPTION=<id>
  tools/qf council [RUN_ID=<run-id>]
  tools/qf arbiter [RUN_ID=<run-id>]
  tools/qf slice [RUN_ID=<run-id>]
  tools/qf discuss [RUN_ID=<run-id>] [PROJECT_ID=<project-id>] [OPTION=<id>] [AUTO_CHOOSE=0|1] [TARGET=prepare|do] [CONFIRM_CONTRACT=0|1] [AUTO_CONFIRM_CONTRACT=0|1]
  tools/qf execute [RUN_ID=<run-id>] [PROJECT_ID=<project-id>] [OPTION=<id>] [AUTO_CHOOSE=0|1] [TARGET=prepare|do] [CONFIRM_CONTRACT=0|1] [AUTO_CONFIRM_CONTRACT=0|1]
  tools/qf review [RUN_ID=<run-id>] [AUTO_FIX=0|1] [STRICT=0|1]
...
emit_json_event() {
  ...
  printf '{"ts":"%s","type":"qf_event","phase":"%s","action":"%s","status":"%s","message":"%s"}\n' \
    "$(date -Iseconds)" \
    "$(json_escape "$phase")" \
    "$(json_escape "$action")" \
    "$(json_escape "$status")" \
...
directions = [
    ...
    },
    {
        ...
        "scope_hint": ["tools/qf", "docs/PROJECT_GUIDE.md", "docs/WORKFLOW.md"],
    ...
    },
    {
        ...
        "scope_hint": ["tools/qf", "docs/WORKFLOW.md", "AGENTS.md", "chatlogs/discussion/"],
    ...
    },
    {
        ...
        "scope_hint": ["tools/qf", "reports/<RUN_ID>/"],
    ...
    },
    {
        ...
        "scope_hint": ["tools/qf", "tests/", "docs/WORKFLOW.md"],
...
next_cmd = f"tools/qf choose RUN_ID={run_id} OPTION={recommended}" if recommended else f"tools/qf choose RUN_ID={run_id} OPTION=<id>"
...
require_council_gate() {
  ...
  if [[ -z "$council_file" ]]; then
    ...
    echo "Run: tools/qf council RUN_ID=${run_id}" >&2
    echo "Then: tools/qf arbiter RUN_ID=${run_id}" >&2
    exit 1
  fi
...
require_direction_gate() {
  ...
  if [[ ! -f "$choice_file" ]]; then
    ...
    echo "Run: tools/qf orient RUN_ID=${run_id}" >&2
    echo "Then: tools/qf choose RUN_ID=${run_id} OPTION=<id>" >&2
...
require_execution_contract_gate() {
  ...
  if [[ ! -f "$file" ]]; then
    ...
    echo "Run: tools/qf arbiter RUN_ID=${run_id}" >&2
    echo "Then: tools/qf slice RUN_ID=${run_id}" >&2
    echo "Then retry: tools/qf do queue-next" >&2
    exit 1
  fi
...
require_slice_gate() {
  ...
  if [[ ! -f "$file" ]]; then
    ...
    echo "Run: tools/qf slice RUN_ID=${run_id}" >&2
    echo "Then retry: tools/qf do queue-next" >&2
    exit 1
  fi
...
if orient_path.is_file():
    try:
        ...
        recommended_option = ""
if not learn_exists:
    next_command = "tools/qf learn"
elif not ready_exists:
    next_command = "tools/qf ready"
elif not choose_exists:
    if orient_path.is_file() and recommended_option:
        next_command = f"tools/qf choose RUN_ID={run_id} OPTION={recommended_option}"
    elif orient_path.is_file():
        next_command = f"tools/qf choose RUN_ID={run_id} OPTION=<id>"
    else:
        next_command = f"tools/qf orient RUN_ID={run_id}"
elif not council_exists:
    next_command = f"tools/qf council RUN_ID={run_id}"
elif not execution_contract_exists:
    next_command = f"tools/qf arbiter RUN_ID={run_id}"
elif not slice_exists:
    next_command = f"tools/qf slice RUN_ID={run_id}"
else:
    next_command = "tools/qf do queue-next"
...
obj = {
    ...
    "next_command_low_friction": (
        f"tools/qf execute RUN_ID={run_id}"
        if ready_exists
        else ("tools/qf ready" if learn_exists else "tools/qf learn")
    ),
}
...
PY
  ...
  append_execution_event "$run_id" "sync" "sync_generated" "ok" "tools/qf sync RUN_ID=${run_id}" "sync_file=${sync_file};sync_md=${sync_md}" ""
...
cmd_learn() {
  ...
  if [[ -n "$py_bin" && -f "tools/qf_learn.py" ]]; then
    "$py_bin" tools/qf_learn.py "$@"
    return $?
  fi
  ...
  local pty_capable="0"
  local model_timeout_sec="${QF_LEARN_MODEL_TIMEOUT_SEC:-300}"
  ...
  for token in "$@"; do
    ...
    case "$token" in
      ...
      PLAN_TRANSPORT=*)
        ...
        ;;
      MODEL_TIMEOUT_SEC=*)
        model_timeout_sec="${token#MODEL_TIMEOUT_SEC=}"
        ;;
      MODEL=*)
  ...
  case "${plan_transport_raw,,}" in
    auto|slash|exec)
      ...
      ;;
    *)
      echo "ERROR: invalid QF_LEARN_PLAN_TRANSPORT=${plan_transport_raw} (expected auto|slash|exec)." >&2
      exit 2
      ;;
  esac
  ...
  if [[ ! "$model_timeout_sec" =~ ^[0-9]+$ || "$model_timeout_sec" -lt 1 ]]; then
    echo "ERROR: invalid MODEL_TIMEOUT_SEC=${model_timeout_sec} (expected positive integer)." >&2
    exit 2
  fi
  case "${log_enabled,,}" in
  ...
  if command -v python3 >/dev/null 2>&1; then
    ...
    echo "ERROR: python is required for tools/qf learn." >&2
    exit 1
  fi
...
PY
  ...
  echo "LEARN_MODEL_TIMEOUT_SEC: ${model_timeout_sec}"
...
lines.append("- Do not call tools/qf init/learn/ready inside this model-sync pass.")
...
PY
    ...
    if [[ "$plan_transport_effective" == "slash" ]]; then
      "$py_bin" - <<'PY' "$model_prompt_file" "$model_raw_file" "$model_events_file" "$model_stderr_file" "$model_timeout_sec" "$model_name"
import json
import os
import pty
import re
...
prompt_file, raw_file, events_file, stderr_file, timeout_sec, model_name = sys.argv[1:]
...
PY
  ...
  echo "LEARN_NEXT_COMMAND: tools/qf ready"
}
...
cmd_ready() {
  ...
  if [[ -n "$py_bin" && -f "tools/qf_ready.py" ]]; then
    "$py_bin" tools/qf_ready.py "$@"
    return $?
  fi
  ...
  if [[ -z "$run_id" ]]; then
    ...
    echo "Usage: tools/qf ready [RUN_ID=<run-id>] [DECISION=resume-close|abandon-new]" >&2
    exit 2
  fi
  ...
  if [[ "$require_learn" == "1" ]]; then
    ...
    if [[ -z "$learn_report_file" && "$auto_learn" == "1" ]]; then
      echo "LEARN_AUTO_RUN: tools/qf learn"
      cmd_learn "PROJECT_ID=${project_id}"
      learn_report_file="$(resolve_learn_file_for_project "$project_id" || true)"
    fi
    if [[ -z "$learn_report_file" ]]; then
      echo "ERROR: learn gate not satisfied." >&2
      echo "Run: tools/qf learn PROJECT_ID=${project_id}" >&2
      echo "Then retry: tools/qf ready RUN_ID=${run_id}" >&2
      exit 1
    fi
  fi
  ...
  if [[ "$resolution_required" -eq 1 ]]; then
    if [[ -z "$continue_decision" ]]; then
      ...
      echo "READY_NEXT_1: tools/qf resume RUN_ID=${run_id}"
      echo "READY_NEXT_2: tools/qf ready RUN_ID=${run_id} DECISION=abandon-new"
      exit 1
    fi
    case "$continue_decision" in
      resume-close)
        echo "READY_DECISION: resume-close"
        echo "READY_NEXT_COMMAND: tools/qf resume RUN_ID=${run_id}"
        exit 1
        ;;
      abandon-new)
        ;;
...
obj = {
    ...
    "understanding": {
        ...
        "suggested_next_step": f"tools/qf orient RUN_ID={run_id}",
    },
}
...
PY
  ...
  append_execution_event "$run_id" "ready" "ready_passed" "ok" "tools/qf ready" "ready_file=${ready_file};learn_report=${learn_report_file};sync_report=${sync_report_file};task_file=${task_file};brief=${ready_brief_md};orient=${orient_file}" ""
...
cmd_orient() {
  ...
  if [[ -n "$py_bin" && -f "tools/qf_orient.py" ]]; then
    "$py_bin" tools/qf_orient.py "$@"
    return $?
  fi
  ...
  append_execution_event "$run_id" "orient" "orient_generated" "ok" "tools/qf orient RUN_ID=${run_id}" "orient_file=${orient_file};orient_md=${orient_md}" ""
...
cmd_choose() {
  ...
  if [[ -n "$py_bin" && -f "tools/qf_choose.py" ]]; then
    "$py_bin" tools/qf_choose.py "$@"
    return $?
  fi
...
contract = {
    ...
    "next_command": "tools/qf council RUN_ID={}".format(obj.get("run_id") or ""),
}
out = {
    "project_id": project_id,
    "run_id": obj.get("run_id"),
    ...
    "next_command": f"tools/qf council RUN_ID={obj.get('run_id')}",
}
...
lines.append("")
lines.append("## Next Command")
lines.append(f"- `tools/qf council RUN_ID={obj.get('run_id')}`")
lines.append("")
Path(contract_md).write_text("\n".join(lines), encoding="utf-8")

print(f"CHOOSE_OPTION: {selected}")
print(f"CHOOSE_TITLE: {picked.get('title', '')}")
print(f"CHOOSE_NEXT_COMMAND: tools/qf council RUN_ID={obj.get('run_id')}")
...
PY
  ...
  append_execution_event "$run_id" "orient" "orient_chosen" "ok" "tools/qf choose RUN_ID=${run_id} OPTION=${option}" "choice_file=${choice_file};contract=${contract_json}" ""
...
cmd_council() {
  ...
  if [[ -n "$py_bin" && -f "tools/qf_council.py" ]]; then
    "$py_bin" tools/qf_council.py "$@"
    return $?
  fi
  ...
  if [[ ! -f "$contract_json" ]]; then
    ...
    echo "Run: tools/qf choose RUN_ID=${run_id} OPTION=<id>" >&2
    exit 1
  fi
...
lines.append("")
lines.append("## Next Command")
lines.append(f"- `tools/qf arbiter RUN_ID={run_id}`")
lines.append("")
Path(out_md).write_text("\n".join(lines), encoding="utf-8")
...
print(f"COUNCIL_NEXT_COMMAND: tools/qf arbiter RUN_ID={run_id}")
...
PY
  ...
  append_execution_event "$run_id" "council" "council_generated" "ok" "tools/qf council RUN_ID=${run_id}" "choice_file=${choice_file};council=${council_json}" ""
...
cmd_arbiter() {
  ...
  if [[ -n "$py_bin" && -f "tools/qf_arbiter.py" ]]; then
    "$py_bin" tools/qf_arbiter.py "$@"
    return $?
  fi
    ...
    tasks.append(
        {
            ...
            "scope": ["tools/qf", "tests/", "chatlogs/discussion/", "reports/{RUN_ID}/"],
...
lines.append("")
lines.append("## Next Command")
lines.append(f"- `tools/qf slice RUN_ID={run_id}`")
lines.append("")
Path(out_md).write_text("\n".join(lines), encoding="utf-8")
...
print(f"ARBITER_NEXT_COMMAND: tools/qf slice RUN_ID={run_id}")
...
PY
  ...
  append_execution_event "$run_id" "arbiter" "arbiter_generated" "ok" "tools/qf arbiter RUN_ID=${run_id}" "council_file=${council_file};execution_contract=${out_json}" ""
...
cmd_slice() {
  ...
  if [[ -n "$py_bin" && -f "tools/qf_slice.py" ]]; then
    "$py_bin" tools/qf_slice.py "$@"
    return $?
  fi
...
print("SLICE_NEXT_COMMAND: tools/qf do queue-next")
...
PY
  ...
  append_execution_event "$run_id" "slice" "slice_generated" "ok" "tools/qf slice RUN_ID=${run_id}" "execution_contract=${execution_contract};slice_state=${out_json}" ""
...
cmd_execute() {
  ...
  append_execution_event "$run_id" "execute" "execute_start" "ok" "tools/qf execute RUN_ID=${run_id}" "project_id=${project_id};option=${option};auto_choose=${auto_choose};confirm_contract=${confirm_contract};auto_confirm_contract=${auto_confirm_contract};target=${target}" ""
  ...
  if [[ -z "$orient_file" || ! -f "$orient_file" ]]; then
    append_execution_event "$run_id" "execute" "execute_orient_missing" "fail" "tools/qf execute RUN_ID=${run_id}" "" "orient file missing after orient step"
    ...
    if [[ -z "$option" ]]; then
      if [[ "$auto_choose" == "1" && -n "$recommended" ]]; then
        ...
        if [[ -n "$recommended" ]]; then
          ...
          echo "EXECUTE_NEXT_COMMAND: tools/qf choose RUN_ID=${run_id} OPTION=${recommended}"
        else
          echo "EXECUTE_NEXT_COMMAND: tools/qf choose RUN_ID=${run_id} OPTION=<id>"
        fi
        append_execution_event "$run_id" "execute" "execute_wait_option" "fail" "tools/qf execute RUN_ID=${run_id}" "recommended=${recommended}" "missing option confirmation"
        exit 1
      fi
    fi
...
PY
  ...
  if [[ "$target" == "do" && ! -f "$contract_confirm_file" ]]; then
    ...
    echo "EXECUTE_NEXT_COMMAND: tools/qf execute RUN_ID=${run_id} PROJECT_ID=${project_id} CONFIRM_CONTRACT=1 TARGET=do"
    append_execution_event "$run_id" "execute" "execute_wait_contract_confirm" "fail" "tools/qf execute RUN_ID=${run_id}" "project_id=${project_id};contract_confirm_file=${contract_confirm_file}" "missing execution contract confirmation"
    exit 1
  fi

  if [[ "$target" == "prepare" ]]; then
    append_execution_event "$run_id" "execute" "execute_prepared" "ok" "tools/qf execute RUN_ID=${run_id} TARGET=prepare" "project_id=${project_id};contract_confirm_file=${contract_confirm_file}" ""
    echo "EXECUTE_STATUS: prepared"
    echo "EXECUTE_NEXT_COMMAND: tools/qf do queue-next"
    return 0
  fi

  echo "EXECUTE_STEP: do"
  RUN_ID="$run_id" cmd_do "queue-next"
  append_execution_event "$run_id" "execute" "execute_done" "ok" "tools/qf execute RUN_ID=${run_id}" "project_id=${project_id};contract_confirm_file=${contract_confirm_file}" ""
}
...
next_cmd = f"tools/qf do queue-next" if status == "pass" else f"tools/qf snapshot RUN_ID={run_id} NOTE=\"review blockers\""
...
PY
  ...
  if [[ "$rc" -ne 0 ]]; then
    append_execution_event "$run_id" "review" "review_failed" "fail" "tools/qf review RUN_ID=${run_id} AUTO_FIX=${auto_fix} STRICT=${strict}" "task_file=${task_file}" "$output"
    append_conversation_checkpoint "$run_id" "review" "review failed; see drift_review.md"
    return "$rc"
  fi

  append_execution_event "$run_id" "review" "review_passed" "ok" "tools/qf review RUN_ID=${run_id} AUTO_FIX=${auto_fix} STRICT=${strict}" "task_file=${task_file}" ""
...
if not ready:
    ...
    if os.path.exists(orient_file):
        next_cmd = f"tools/qf choose RUN_ID={run_id} OPTION=<id>"
    else:
        next_cmd = f"tools/qf orient RUN_ID={run_id}"
elif not os.path.exists(council_file):
    next_cmd = f"tools/qf council RUN_ID={run_id}"
elif not os.path.exists(execution_contract_file):
    next_cmd = f"tools/qf arbiter RUN_ID={run_id}"
elif not os.path.exists(slice_state_file):
    next_cmd = f"tools/qf slice RUN_ID={run_id}"
else:
    next_cmd = "tools/qf do queue-next"
...
cmd_init() {
  ...
  if [[ -n "$py_bin" && -f "tools/qf_init.py" ]]; then
    "$py_bin" tools/qf_init.py "$@"
    return $?
  fi
  ...
  if [[ "$init_status" != "ok" ]]; then
    if [[ -n "$current_run_id" ]]; then
      next_cmd="tools/qf resume RUN_ID=${current_run_id}"
    else
      next_cmd="tools/qf resume"
    fi
  fi
  ...
  if [[ "$init_mode" == "check" ]]; then
    echo "== 建议流程 =="
    echo "1) tools/qf learn"
    echo "2) tools/qf ready"
    echo "3) tools/qf discuss TARGET=prepare"
    echo "4) tools/qf do queue-next"
  else
    echo "== 状态查询完成 =="
  fi
}
...
cmd_do() {
  ...
  if is_dirty; then
    ...
    append_execution_event "$run_id" "do" "do_sync_skipped_dirty" "ok" "tools/qf do queue-next" "reason=dirty_worktree" ""
  else
    sync_main "$run_id"
  fi
  append_execution_event "$run_id" "do" "do_start" "ok" "tools/qf do queue-next" "ready_file=${ready_file};choice_file=${choice_file};council_file=${council_file};execution_contract=${execution_contract};slice_state=${slice_state}" ""
  ...
  if [[ "$rc" -ne 0 ]]; then
    ...
    if [[ "$output" == *"QUEUE 中没有未完成项"* ]]; then
      echo "下一步建议：tools/qf orient RUN_ID=${run_id}" >&2
    fi
    print_resume_cmd "$run_id"
    exit "$rc"
  fi
  ...
  if [[ "${QF_DO_AUTO_REVIEW:-1}" == "1" ]]; then
    echo "AUTO_REVIEW: tools/qf review RUN_ID=${review_run_id} AUTO_FIX=${QF_DO_REVIEW_AUTO_FIX:-1}"
    set +e
    ...
    if [[ "$review_rc" -ne 0 ]]; then
      append_execution_event "$review_run_id" "do" "do_auto_review_failed" "fail" "tools/qf review RUN_ID=${review_run_id}" "" "$review_output"
      echo "WARN: auto review failed (non-blocking)."
    else
      append_execution_event "$review_run_id" "do" "do_auto_review_done" "ok" "tools/qf review RUN_ID=${review_run_id}" "" ""
    fi
  fi
}
...
is_qf_stash_cleanup_candidate() {
  local subject="${1:-}"
  case "$subject" in
    *": ship-wip-"*|*": qf-init-wip-"*|*": resume-cleanup-run-"*|*": qf-resume-cleanup-run-"*|*": tmp-ship-cleanup-"*)
      return 0
      ;;
    *)
      return 1
...
- 证据文件路径：`AGENTS.md`、`tools/qf`、`reports/{run_id}/conversation.md`
...
- 操作指南路径：`docs/WORKFLOW.md`
- 证据文件路径：`tools/qf`、`chatlogs/discussion/{run_id}/`
...
- 证据文件路径：`tools/qf`、`chatlogs/discussion/{run_id}/council.json`
...
PY
    then
      ...
      append_execution_event "$run_id" "exam-auto" "exam_auto_answer_migrated" "ok" "tools/qf exam-auto RUN_ID=${run_id}" "answer_file=${answer_file}" ""
